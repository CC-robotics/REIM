...
def plot_framework(output_path: str | Path) -> Path:
    ...
    def box(
        ...
        *,
        ...
        ax.add_patch(patch)
        ax.text(x, y, text, ha="center", va="center", fontsize=9, weight=weight)
        return patch
    ...
    # Flat, low-noise zones encode the two distinct phases.  The dashed upper
    # path is never present at evaluation time.
    ax.add_patch(
        Rectangle(
            (0.2, 3.75),
            13.6,
    ...
    ax.text(
        0.42,
        6.25,
        "TRAINING ONLY  •  TRIGGER-ALIGNED CORRECTIVE IMITATION",
        fontsize=7.7,
        color=MID_GRAY,
        weight="semibold",
    )
    ...
    ax.text(
        0.42,
        3.25,
        "DEPLOYMENT  •  CLOSED-LOOP ARBITRATION",
        fontsize=7.7,
        color=MID_GRAY,
        weight="semibold",
    )

    # Training curriculum: disturbed nominal rollouts identify deployment-aligned
    # trigger states; successful scripted continuations provide corrective labels.
    box(
        (1.55, 5.16),
        (2.35, 0.92),
        "Disturbed ACT rollout\n+ causal LSTM",
        face=PALE_BLUE,
        edge=BLUE,
    )
    box(
    ...
    box(
        (6.95, 5.16),
        (2.5, 1.08),
        "Exact MuJoCo snapshot\n$q,\\dot q$, mocap, control,\ngoal + task bookkeeping",
        face=PALE_TEAL,
        edge=TEAL,
    )
    box(
    ...
    for start, end, color in (
        ((2.74, 5.16), (3.02, 5.16), ORANGE),
        ((5.36, 5.16), (5.66, 5.16), TEAL),
        ((8.22, 5.30), (8.42, 5.60), TEAL),
        ((8.22, 5.01), (8.42, 4.68), TEAL),
        ((10.50, 5.69), (10.62, 5.69), ORANGE),
        ((10.50, 4.61), (10.62, 4.61), ORANGE),
    ):
        arrow(start, end, color=color, linestyle="--")
    arrow(
        (11.55, 5.30),
        (11.55, 4.99),
        color=ORANGE,
    ...
    ax.text(
        7.0,
        4.12,
        "Trigger-aligned train/validation seed banks are disjoint",
        ha="center",
        fontsize=7.2,
        color=MID_GRAY,
    )
    ...
    box((1.32, 1.92), (1.62, 0.8), "Robot state\n$s_t$", face=WHITE, edge=TEAL)
    box(
        (3.34, 2.55),
        (2.0, 0.82),
        "ACT policy\n20-action chunks",
    ...
    diamond = Polygon(
        ((5.45, 2.50), (6.25, 1.92), (5.45, 1.34), (4.65, 1.92)),
        closed=True,
    ...
    box(
        (12.0, 1.92),
        (2.25, 1.02),
        "Meta-World PickPlace\nMuJoCo Sawyer (sim.)",
        face=PALE_TEAL,
        edge=TEAL,
    )

    arrow((2.14, 2.10), (2.32, 2.42), color=BLUE)
    arrow((2.14, 1.73), (2.32, 1.42), color=ORANGE)
    arrow((4.35, 1.31), (4.77, 1.57), color=ORANGE)
    arrow((6.05, 2.15), (6.34, 2.43), color=BLUE)
    ax.text(6.18, 2.46, "No", fontsize=7.3, color=BLUE)
    arrow((5.75, 1.48), (6.25, 1.24), color=ORANGE)
    ax.text(5.99, 1.18, "Yes", fontsize=7.3, color=ORANGE)
    arrow((8.18, 2.55), (8.53, 2.13), color=BLUE)
    arrow((8.23, 1.29), (8.53, 1.68), color=ORANGE)
    arrow((10.34, 1.92), (10.86, 1.92), color=BLUE)

    # The recovery option is deliberately persistent.  There is no
    # risk-clear edge back to ACT in the frozen protocol.
    ax.text(
        7.25,
        0.61,
        "hold recovery until task success or 150-step budget\n(no mid-recovery release)",
        ha="center",
        va="center",
        fontsize=7.4,
        color=ORANGE,
    ...
    ax.text(
        7.0,
        6.90,
        "REIM: Recovery-Enhanced Imitation Learning",
        ha="center",
        va="top",
        fontsize=12,
        weight="semibold",
    )
    result = _save(fig, output_path, title="REIM closed-loop architecture")
    plt.close(fig)
    return result
