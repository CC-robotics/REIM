...
The experiments show that learned risk arbitration is most useful when perturbations push nominal policies far outside their training distribution. REIM increasingly outperforms reward-stagnation gating as disturbance strength grows, and this pattern holds across MT10 and MT50 and across component-training seeds. The detector's value lies in coupling early risk estimates to a recovery policy through temporally constrained arbitration rather than treating risk classification as an endpoint. Hysteresis turns the score into stable control decisions, while paired analysis identifies where mistimed interventions still reduce performance. The central opportunity is therefore to retain the strong-noise gains while lowering unnecessary recovery occupancy.

The present evidence is limited to known-task Meta-World simulation, separately trained MT10 and MT50 models, and Gaussian state--action perturbations. It does not establish unseen-task or real-robot transfer, and successful expert continuations may not cover every contact-rich recovery state. Structured disturbances, occupancy-matched comparisons, and physical-robot evaluation are the most direct extensions of this study.
