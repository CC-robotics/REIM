"""Run the frozen MT50 occupancy-matched heuristic confirmation in shards."""
...
def main() -> None:
    ...
    stem = "mt50_confirmation_heuristic_occ_match"
    ...
    for shard in range(workers):
        ...
        argv = [
            ...
            "--benchmark", "MT50",
            "--condition", "robustness_noise_40",
            "--benchmark-seed", "20266050",
    ...
    for shard, process, console in processes:
        ...
        console.close()
        print(f"finished shard {shard}: return_code={return_code}")
        if return_code != 0:
    ...
    merge_argv = [str(PYTHON), "scripts/merge_multitask_evaluation_shards.py"]
    for shard in range(workers):
        merge_argv.extend(
            (
                "--shard",
                str(OUT / f"{stem}_shard{shard}_summary.json"),
                str(OUT / f"{stem}_shard{shard}_episodes.csv"),
            )
        )
    merge_argv.extend(
        ("--output-csv", str(merged_csv), "--output-summary", str(merged_summary), "--overwrite", "--allow-method-subset")
    )
    subprocess.run(merge_argv, cwd=ROOT, check=True)
