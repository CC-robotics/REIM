...
Figure~\ref{fig:framework} overviews the motivating failure mode, the
risk-gated controller, trigger-aligned recovery training, and the primary
paired outcome.
...
\begin{figure*}[t]
  \centering
  \PaperGraphic{Figure1_overview}{0.90\textwidth}{%
  Three-panel overview of ACT failure after an object displacement,
  risk-gated nominal or recovery control, trigger-aligned recovery training,
  and the paired PickPlace outcome.}
  \caption{Overview of REIM. (a) After a simulated object displacement, ACT
  continues its current action chunk and times out. (b) The causal LSTM
  estimates risk from the last 10 states; the frozen gate retains ACT when
  $p_t<\tau_{\mathrm{on}}=0.20$ and activates the persistent recovery actor
  otherwise. The lower path summarizes trigger-aligned Smooth L1 training from
  successful expert continuations. (c) Representative recovery frames and the
  paired PickPlace result at 20\% mixed disturbance
  ($n=\PlannedMainEpisodes{}$): \FinalREIMSuccess{} for REIM versus
  \FinalACTSuccess{} for ACT, a difference of \FinalREIMGain{}
  (\ConfidenceLevel{} CI \FinalREIMGainCI{}). All robot images show the
  simulated Sawyer model in Meta-World/MuJoCo.}
  \label{fig:framework}
\end{figure*}
...
Figure~\ref{fig:final-results} and Table~\ref{tab:final-baseline} summarize
the paired PickPlace comparison. REIM achieves \FinalREIMSuccess{} task
...
persists at matched burden. A qualitative paired rollout under the frozen
evaluation gate is visualized in Fig.~\ref{fig:operation}.
...
At the high-threshold end ($\tau_{\mathrm{on}} = 0.60$), success drops to
77.0\% and the post-intervention recovery rate collapses to 2.1\%: the
detector rarely fires before the episode has already deteriorated, so the
recovery option arrives too late to help. The sweep therefore supports the
frozen 0.20 operating point rather than a higher trigger.

\begin{figure*}[t]
  \centering
  \PaperGraphic{Figure2_final_results}{0.88\textwidth}{%
...
\begin{figure}[t]
  \centering
  \PaperGraphic{Figure4_gate_sensitivity}{0.90\columnwidth}{%
  Success--intervention sensitivity across six LSTM thresholds.}
  ...
  frozen primary gate, the dashed comparison at 0.175 exposes matched
  intervention burden, and the $\tau_{\mathrm{on}} = 0.60$ endpoint shows that
  very late triggers forfeit most of the recovery benefit. This diagnostic
  was not used for model or threshold selection.}
  \label{fig:gate-sensitivity}
\end{figure}

\subsection{Known-Task Multi-Task Breadth}

\input{Table_multitask_clean.tex}
...
\MTFiftyREIMDeltaVsACTCI) on \MTFiftyBenchmark{}.
Figure~\ref{fig:multitask-robustness} reports the nonzero-disturbance results
separately as a robustness extension; they are not described as
...
The supervised recovery gate is also stable across training seeds: on the MT10
bank (20265010, 50 episodes per task), seeds 42/43/44 give task-macro success
of $96.6 \pm 0.4$\% clean and $60.8 \pm 4.9$\% at 40\% disturbance, versus
$96.7 \pm 0.9$\% and $40.8 \pm 3.6$\% for the heuristic gate (mean $\pm$ s.d.
over the three seeds), so seed-to-seed variation is small relative to the
disturbance gap.

\begin{figure*}[t]
  \centering
  \PaperGraphic{Figure_multitask_robustness}{0.86\textwidth}{%
...
\input{Table3_component_diagnostics.tex}

Table~\ref{tab:component-diagnostics} collects the completed component
diagnostics. ACT reaches validation action L1 \ACTBestValidationLone{} on a
...
\begin{figure}[t]
  \centering
  \PaperGraphic{Figure3_detector}{0.72\columnwidth}{%
...
\begin{figure*}[t]
  \centering
  \PaperGraphic{Figure5_operation_sequence}{0.78\textwidth}{%
...
Figure~\ref{fig:ablation} visualizes the component-removal study.
...
\begin{figure}[t]
  \centering
  \PaperGraphic{Figure3_final_ablation}{0.85\columnwidth}{%
...
A training-design control on 200 paired development seeds at 20\% disturbance
further shows that the recovery recipe matters: exact trigger-state resets with
an expert-warm-started actor reach 91.5\%, while removing the warm start drops
to 54.5\% and an unselected 500k training endpoint reaches 75.5\%.
