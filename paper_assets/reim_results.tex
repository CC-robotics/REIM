...
\begin{itemize}
  ...
  \item a reproducible paired simulation protocol that separates classifier
  diagnostics, model-selection validation, intervention-conditioned task
  completion, final task success, and controller intervention burden.
\end{itemize}
...
$\policyact$, risk predictor $f_\phi$, and recovery policy $\policyrec$.
...
Experiments use the Meta-World 3.1.1 \REIMTask{} single-task,
...
The main evaluation uses \PlannedMainEpisodes{} paired CRN episode
...
frozen LSTM gate Pareto-dominates this simulator-privileged heuristic in the
primary CRN benchmark; the paired gate audit below tests whether this persists
at matched burden.
...
The current study has several boundaries. First, it evaluates one
state-observed Meta-World task; visual perception, multi-task transfer, and
contact sensing are outside scope. Second, exact simulator restoration depends
on privileged MuJoCo state and task bookkeeping. It is a controlled way to
align recovery training in simulation, not a deployable physical-robot reset
mechanism. Hardware would require state estimation and safe physical
repositioning or real failure collection. Third, filtering recovery-imitation
...
on disturbed rollouts from one nominal policy and may shift as ACT or the
disturbance distribution changes. Moreover, the inclusive label makes the
standard confusion matrix predominantly a current-event diagnostic, and
...
is conditional on one trained ACT, detector, and supervised recovery actor, so
it does not propagate all model-training uncertainty. Sixth, the validation
...
reinforcement-learning interaction. In paired
Meta-World/MuJoCo evaluation, REIM improves ACT by \FinalREIMGain{} and remains
